#!/usr/bin/env python3
"""
Analyze Depth Data Clusters - VPS Version
Finds significant price levels with order concentrations from today's depth data

Run on VPS: python3 analyze_depth_clusters_vps.py
"""
...
def analyze_price_clusters(rows, min_orders=20):
    ...
    if all_orders:
        avg_all = sum(all_orders) / len(all_orders)
        # Simple stdev calculation
        variance = sum((x - avg_all) ** 2 for x in all_orders) / len(all_orders)
